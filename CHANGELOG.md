...
- Added support for [tip-eligible amounts](https://docs.stripe.com/terminal/features/collecting-tips/on-reader#tip-eligible) on WisePad 3 devices, allowing configuration of the portion of a transaction amount on which tips are calculated.
...
#### Updates
- Devices are now required to have an Android security patch update from the last 12 months to use Tap to Pay. Devices with outdated security patches will fail to connect to the Tap to Pay reader.
    - Check device security status in Settings. Update schedules vary by manufacturer; see [support guides](https://support.google.com/android/answer/7680439) for details.
